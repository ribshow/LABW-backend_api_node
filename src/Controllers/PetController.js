...
export default class PetController {
  static async create(req, res) {
    ...
    try {
      ...
      res
        .status(201)
        .json({ message: "Pet cadastrado com sucesso!", petCreated });
    } catch (error) {
  ...
  static async getAll(req, res) {
    try {
      const pets = await Pet.find({ situation: { $ne: "Finalizado" } }).sort(
        "-createdAt"
      );
  ...
  static async getAllByUser(req, res) {
    ...
    try {
      // sort ordena do mais recente para o mais antigo
