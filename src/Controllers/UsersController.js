...
export default class UserController {
  static async register(req, res) {
    ...
    if (!name) {
      ...
      res
        .status(422)
        .json({ error: true, message: "O campo email é obrigatório!" });
      return;
    } else if (!password) {
      res
        .status(422)
        .json({ error: true, message: "O campo senha é obrigatório!" });
      return;
  ...
  static async login(req, res) {
    ...
    try {
      ...
      if (!password)
        return res.status(422).json({ message: "Senha é requerido" });
  ...
  static async notification(req, res) {
    ...
    try {
      await sendMessage("+5514998283036", message);
